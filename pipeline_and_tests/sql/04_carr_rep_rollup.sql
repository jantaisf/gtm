...
-- Aggregates cACV from account level to rep and region level.
-- Used by the executive dashboard and sales compensation tracking.
-- ─────────────────────────────────────────────────────────────────────────────

CREATE OR REPLACE TABLE `openclaw-gateway-491103.gtm.cacv_rep_rollup` AS
...
WITH rep_metrics AS (
  SELECT
    ...
    -- ARR vs cACV (exclude ramping accounts from cACV sum)
    ...
    -- cACV attainment rate = cACV / ARR (on mature accounts only)
    ROUND(
      SAFE_DIVIDE(
        SUM(ca.cacv),
    ...
    SUM(CASE WHEN ca.expansion_flag
        THEN ca.annual_commit_dollars ELSE 0 END)               AS expansion_acv_pipeline,
