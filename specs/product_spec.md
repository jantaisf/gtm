...
| **Cohort Consumption ACV** | Sum of account Consumption ACV grouped by contract start quarter and/or account segment (e.g., all Enterprise accounts signed in Q1 FY2025) — see note below |

> **Cohort aggregation for onboarding trend monitoring:** Grouping accounts by their contract start quarter creates a cohort view that is distinct from the point-in-time snapshots above. Rather than asking "what is the portfolio's consumption rate today?", a cohort view asks "for accounts that signed in Q1 FY2025, how has their average consumption rate evolved month by month since go-live?" This is the primary tool for detecting systematic onboarding problems: if every cohort follows a similar ramp curve, the pipeline and CS process are working; if a specific cohort stalls at a lower plateau than prior cohorts, something changed — a product release, a segment mix shift, a comp plan change, or a CS capacity constraint.
>
> **Segment-level cohorts are a priority hypothesis.** Enterprise, Mid-Market, and SMB accounts are likely to exhibit materially different ramp curves — Enterprise deployments span multiple business units and compliance domains and may take 6–9 months to reach steady-state consumption, while SMB accounts may ramp faster but also drop off faster if the initial deployment is thin. If these patterns are not separated, a growing SMB mix can mask a deteriorating Enterprise ramp (or vice versa), and any single activation target or health tier threshold will be miscalibrated for at least one segment. The recommended approach is to track cohort curves by segment × start quarter from day one, even if the v1 dashboard only surfaces the blended view. Segment-specific thresholds (e.g., different ramp window lengths or activation bonus timelines for Enterprise vs. SMB) are a v2 calibration item once sufficient per-segment data has accumulated. See §2.2.2 Key Assumptions.
>
> Cohort analysis is also the correct unit for validating health tier thresholds against actual renewal outcomes (see §2.6 v1 note). v1 stores sufficient data to compute cohort aggregations by both start quarter and segment; surfacing them in a dedicated dashboard view is a v2 consideration.
